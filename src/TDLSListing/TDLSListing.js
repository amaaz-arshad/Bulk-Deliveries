...
import { View, Text, FlatList, ToastAndroid, StatusBar, TouchableOpacity } from 'react-native';
...
export default class TDLSListing extends Component {
            ...
            if (isSubmitSuccessfully) {
                ...
                NetInfo.fetch().then(networkState => {
                    if (!networkState.isInternetReachable)
                        this.saveSubmittedFormData_withDBupdate_onBack(networkState.isInternetReachable)
                    else {
    ...
    componentWillReceiveProps(nextProps) {//to save data locally when sync
        if (this.props !== nextProps) {
            ...
            NetInfo.fetch().then(networkState => {
                ...
                if (!networkState.isInternetReachable) {
                    ...
                    this.setState({ currentTab: this.state.currentTab, dateFrom: '', dateTo: '', isChecked: false, tripPlaceHolder: 'Select Trip No\t' })
        ...
        BluetoothManager.connect(address) // the device address scanned.
            .then((s) => {
                ...
                BluetoothEscposPrinter.printText(
                    ...
                    'TDLS#               ' + this.state.printData.TDLS_No + "\n" +
                    'Date                ' + this.state.printData.ScheduledDate + "\n" +
                    'Customer #          ' + this.state.printData.CustomerNo + "\n" +
                    'Name                ' + this.state.printData.CustomerName + "\n" +
                    'Product code        ' + this.state.printData.PrimaryProduct + "\n" +
                    'Description         ' + this.state.printData.ProductName + "\n" +
                    'Tanker #            ' + this.state.printData.vehicleno + "\n" +
                    'Decanter Name       ' + this.state.printData.DecanterName + "\n" +
                    'Driver Name         ' + this.state.printData.DriverName + "\n" +
                    'Date & Time In      ' + this.state.printData.datein + "\n" +
                    'Odometer In         ' + this.state.printData.odometerin + "\n" +
                    'Date & Time Out     ' + this.state.printData.dateout + "\n" +
                    'Odometer Out        ' + this.state.printData.odometerout + "\n" +
                    'Delivery in         ' + this.state.printData.CalculationBaseType + "\n" +
                    "________________________________________________" + "\n\n"
                    ,
                ...
                BluetoothEscposPrinter.printColumn([20, 9, 9, 9],
                    [BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT],
                    ["Pressure", this.state.printData.vie_press_start + "", this.state.printData.vie_press_end + "", this.state.printData.vie_press_start_unit + ""], {});

                BluetoothEscposPrinter.printColumn([20, 9, 9, 9],
                    [BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT],
                    ["Level", this.state.printData.vie_level_start + "", this.state.printData.vie_level_end + "", this.state.printData.vie_level_end_unit + ""], {});
                ...
                BluetoothEscposPrinter.printColumn([20, 9, 9, 9],
                    [BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT],
                    ["Pressure", this.state.printData.tanker_press_start + "", this.state.printData.tanker_press_end + "", this.state.printData.tanker_press_start_unit + ""], {});

                BluetoothEscposPrinter.printColumn([20, 9, 9, 9],
                    [BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT],
                    ["Level", this.state.printData.tanker_level_start + "", this.state.printData.tanker_level_end + "", this.state.printData.tanker_level_start_unit + ""], {});

                BluetoothEscposPrinter.printColumn([20, 9, 9, 9],
                    [BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.LEFT],
                    ["Content", this.state.printData.tanker_content_start + "", this.state.printData.tanker_content_end + "", this.state.printData.content_start_unit + ""], {});
                ...
                BluetoothEscposPrinter.printText(
                    'Delivered Volume    ' + this.state.printData.tanker_content_diff + "\n" +
                    'Delivered Unit      ' + this.state.printData.uom + "\n" +
                    'Comments:           ' + this.state.printData.remarks + "\n" +
                    'Customer Signature: ' + "" + "\n" +//sig ki image
                    'Printed by:         ' + this.loginId + "\n" +
                    'Print date:         ' + this.state.printData.Date_Out + "\n" +
                    'Device id :         ' + "WOOSIM" + "\n" +
                    'Call on:            ' + "+9211165892525" + "\n" +
    ...
    print() {
        ...
        let outodometer = this.state.printData.odometerout + '\n' +
            '------------------------------------------------' +
            '\n';
        let chk = this.state.printData.CalculationBaseType == null ? '' : this.state.printData.CalculationBaseType + "";
        // let netweight = this.state.printData.tanker_content_diff + " KG"  + '\n';

        let diff = this.state.printData.tanker_content_start - this.state.printData.tanker_content_end;
        let netweight =
        ...
        let deliveredvolume = this.state.printData.tanker_content_diff + " " + this.state.printData.uom + '\n';
        //let deliveredvolume = this.state.printData.tanker_content_diff + " " + 'M3' + '\n';
        let comments = this.state.printData.Additional_Remarks == null ? '\n' : this.state.printData.Additional_Remarks + '\n';
        ...
        console.log('CalculationBaseType: ', this.state.printData.CalculationBaseType)
        console.log('CalculationBaseType id: ', this.state.printData.CalculationBaseTypeID)
    ...
    saveSubmittedFormData_withDBupdate_onBack(isNetAvailable) {
        if (!isNetAvailable) {
            ...
            setTimeout(() => {
                ...
                var index = response.findIndex(e => e.TDLS_No == JSON.parse(responseformData).TDLS_No);
                if (index != -1) {
    ...
    getFiltered_By_TripNo_Status(response, bttrNo, status) {
        const data = response.filter(x => (x.BTTR_No == bttrNo && x.Trip_Status == status));
        return data;
    }
    ...
    sendAllFormsToServer = async () => {
        ...
        setTimeout(() => {
            ...
            arrayData.forEach((data, index) => {
                return axios.post(BASE_URL + setDelivery, data, {
                    "headers": {
                    ...
                    if (response.data.ResultCode == "S") {
                        if (index == arrayData.length - 1) {
                            ToastAndroid.show(response.data.ResultStatus, ToastAndroid.LONG)
                            try {
    ...
    getSchedulerListAll(loginId) {
        ...
        axios.get(BASE_URL + getSchedulerList_DateAndAll,
            {
                params: {
                    userid: loginId,
                    From: '',
            ...
            <View>
                ...
                <View style={styles.dateContainerStyle}>
                    ...
                    <View style={{ flexDirection: 'row', alignContent: 'center' }}>
                        <Text style={{ alignSelf: 'center', marginStart: 15, marginEnd: 5 }}>From</Text>


                        {/* View For Border */}
                        <View style={{
                            marginEnd: 5, borderWidth: 1, borderColor: '#b2b2b2',
                            borderRadius: 3,
                        }}>
                            ...
                            <DatePicker
                                ...
                                onDateChange={(date) => {
                                    this.setState({ dateFrom: date, dateTo: '', toDateMinRange: date, tripPlaceHolder: 'Select Trip No\t' })
                                    this.onDateSelected()
                                    this.setState({ tripPlaceHolder: 'Select Trip No' })
                    ...
                    <View style={{ flexDirection: 'row', alignContent: 'center', }}>
                        ...
                        {/* View For Border */}
                        <View style={{ marginStart: 5, marginEnd: 15, borderWidth: 1, borderColor: '#b2b2b2', borderRadius: 3 }}>
                            ...
                            <DatePicker
                                ...
                                onDateChange={(date) => {
                                    this.setState({ dateTo: date, tripPlaceHolder: 'Select Trip No\t' })
                                    this.onDateSelected()
                                    this.setState({ tripPlaceHolder: 'Select Trip No' })
                                }}
                            />
    ...
    onDateSelected() {
        ...
        if (this.state.currentTab == 0) {
            this.setState({ pendingData: this.getFiltered_Date_Data(this.state.mainResp, this.state.dateFrom, this.state.dateTo, 5) })
            this.setTripNoDropDown(this.state.pendingData, this.state.currentTab)
        } else if (this.state.currentTab == 1) {
            this.setState({ approvedData: this.getFiltered_Date_Data(this.state.mainResp, this.state.dateFrom, this.state.dateTo, 6) })
            this.setTripNoDropDown(this.state.approvedData, this.state.currentTab)
        } else {
            this.setState({ closedData: this.getFiltered_Date_Data(this.state.mainResp, this.state.dateFrom, this.state.dateTo, 4) })
    ...
    checkBoxClick = isChecked => {
        ...
        if (isChecked) {
            this.setState({ dateFrom: '', dateTo: '', tripPlaceHolder: 'Select Trip No\t' })

            if (this.state.currentTab == 0) {
                this.setState({ pendingData: this.getFiltered_Status_Data(this.state.mainResp, 5) })
                this.setTripNoDropDown(this.state.pendingData, this.state.currentTab)
            }

            else if (this.state.currentTab == 1) {
                this.setState({ approvedData: this.getFiltered_Status_Data(this.state.mainResp, 6) })
                this.setTripNoDropDown(this.state.approvedData, this.state.currentTab)
            }
            else {
                this.setState({ closedData: this.getFiltered_Status_Data(this.state.mainResp, 4) })
                this.setTripNoDropDown(this.state.closedData, this.state.currentTab)
            }
    ...
    setTripNoDropDown(response, whichTab) {
        ...
        if (this.state.currentTab == 0) {
            ...
            tabWiseTripNo = 6
        } else { tabWiseTripNo = 4 }


        //for fetching unique trip no/bttr no from response
        const tripNo = this.getFiltered_By_TripStatus(response, tabWiseTripNo).map(x => x.BTTR_No);
    ...
    getSelectedTripNo = val => {
        ...
        if (this.state.currentTab == 0) {
            ...
            if (this.state.dateFrom != '' && this.state.dateTo != '') {
                ...
                const dataFilteredByTripNo = this.getFiltered_By_TripNo_Status(this.state.mainResp, this.state.selectedTripNo, 5)
            ...
            if (this.state.dateFrom != '' && this.state.dateTo != '') {
                ...
                const dataFilteredByTripNo = this.getFiltered_By_TripNo_Status(this.state.mainResp, this.state.selectedTripNo, 6)
            ...
            if (this.state.dateFrom != '' && this.state.dateTo != '') {
                ...
                const dataFilteredByTripNo = this.getFiltered_By_TripNo_Status(this.state.mainResp, this.state.selectedTripNo, 4)
        ...
        return (
            <View style={{ flex: 1, flexDirection: 'column' }}>
                ...
                <Tabs onChangeTab={(tab) => {
                    ...
                    setTimeout(() => {
                        this.setState({ currentTab: tab.i, dateFrom: '', dateTo: '', isChecked: false, tripPlaceHolder: 'Select Trip No\t' })
                    ...
                    <Tab
                        ...
                        heading={<TabHeading style={{ backgroundColor: colors.white }}>
                            <Text style={{ color: colors.primary }}>Pending</Text></TabHeading>}>
                        {this.pendingTab()}
                    ...
                    <Tab
                        style={{ backgroundColor: "#ffffff" }}
                        heading={<TabHeading style={{ backgroundColor: colors.white }}>
                            <Text style={{ color: colors.primary }}>Approved</Text></TabHeading>}>
                    ...
                    <Tab
                        style={{ backgroundColor: "#ffffff" }}
                        heading={<TabHeading style={{ backgroundColor: colors.white }}>
                            <Text style={{ color: colors.primary }}>Rejected</Text></TabHeading>}>
...
const styles = ScaledSheet.create({
    ...
    dropDownViewStyle: {
        borderWidth: 0.5, borderColor: colors.grey2, backgroundColor: colors.white, margin: 15, marginStart: 18, marginEnd: 21, height: 50
    },
    dropDownStyle: {
        height: 45,
